...
채팅 기록은 PostgreSQL에 저장되므로 한쪽이 늦게 접속해도 기존 메시지를 불러옵니다. 채팅방에 `실시간 연결됨`이 표시되면 Socket.IO 연결과 방 참여가 모두 완료된 상태입니다. 연결 또는 응답이 끊기면 같은 메시지 의도 UUID를 사용하는 REST fallback으로 저장하며, 서버 unique 제약이 Socket·REST 중복 저장을 막습니다.
...
| 페이지/상품은 보이지만 실시간 채팅이 연결되지 않음 | `.env`의 `WEB_ORIGIN`이 브라우저 주소와 포트까지 완전히 같은지, Web 이미지를 재빌드했는지, 모든 기기가 같은 LAN URL을 쓰는지 확인합니다. 수정 직후에는 브라우저를 강력 새로고침합니다. |
